...
class _DealerAssignmentScreenState
  ...
  /// Only Admin / Super Admin (handleUser permission) can edit assignments.
  bool _canEdit() => AppPermissions.canAccess(
        ref.read(roleNotifierProvider),
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: SafeArea(child: Column(children: [
          ...
          // ── Bottom bars (only Admin / Super Admin) ───────────────────────
          if (_canEdit()) ...[
            if (_selectionMode)
  ...
  Widget _buildHeader(double sw, double sh) {
    ...
    // Tab 1 — All Dealers — search active
    if (_tabCtrl.index == 1 && _showAllSearch) {
      return Container(
        color: _kWhite,
        padding: EdgeInsets.fromLTRB(
            sw * 0.04, sh * 0.015, sw * 0.04, sh * 0.015),
    ...
    // Tab 0 — Assigned — search active
    if (_tabCtrl.index == 0 && _showAssignedSearch) {
      return Container(
        color: _kWhite,
        padding: EdgeInsets.fromLTRB(
            sw * 0.04, sh * 0.015, sw * 0.04, sh * 0.015),
