...
class _ProductsScreenState extends ConsumerState<ProductsScreen> {
    ...
    return Scaffold(backgroundColor: _kBg,
        body: SafeArea(child: Column(children: [
          ...
          Container(color: _kWhite, child: Column(children: [
            Padding(padding: EdgeInsets.fromLTRB(sw * 0.04, sh * 0.015, sw * 0.04, sh * 0.012),
                child: _showSearch ? _searchBar(sw) : Row(children: [
                  ...
                  const Spacer(),
                  CircularIconButton(icon: Icons.search_rounded,
                      onTap: () => setState(() => _showSearch = true)),
                  SizedBox(width: sw * 0.02),
                  RoleGuard(feature: AppFeature.createProduct,
                  ...
                  itemBuilder: (_, i) {
                    if (i == filtered.length) {
                      return Padding(padding: EdgeInsets.symmetric(vertical: sw * 0.05),
                          child: const Center(child: CircularProgressIndicator(color: _kP, strokeWidth: 2.5)));
                    }
                    final p = filtered[i];
                    return _ProductCard(product: p, sw: sw, sh: sh,
  ...
  Widget _statusChip(double sw, int i) {
    ...
    return GestureDetector(
        onTap: () { setState(() => _tabIdx = i); _scrollChip(i); },
        child: Container(
            margin: EdgeInsets.only(right: sw * 0.02),
            padding: EdgeInsets.symmetric(
                horizontal: (sw * 0.032).clamp(10.0, 16.0),
                vertical: (sw * 0.014).clamp(5.0, 8.0)),
            decoration: BoxDecoration(color: sel ? _kWhite : _kBg,
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: sel ? dot : _kBd, width: sel ? 1.0 : 0.5)),
  ...
  Widget _brandChip(double sw, AsyncValue<List<BrandModel>> brandState) {
    ...
    return GestureDetector(
        onTap: () => _showBrandSheet(context, sw, MediaQuery.sizeOf(context).height, brandState),
        child: Container(
            margin: EdgeInsets.only(right: sw * 0.02),
            padding: EdgeInsets.symmetric(
                horizontal: (sw * 0.032).clamp(10.0, 16.0),
                vertical: (sw * 0.014).clamp(5.0, 8.0)),
            decoration: BoxDecoration(color: has ? _kAmberBg : _kBg,
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: has ? _kAmberBd : _kBd, width: has ? 1.0 : 0.5)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Container(width: (sw * 0.016).clamp(5.0, 8.0),
                  height: (sw * 0.016).clamp(5.0, 8.0),
                  decoration: BoxDecoration(color: has ? _kAmber : _kT4, shape: BoxShape.circle)),
              SizedBox(width: sw * 0.015),
              Text(has ? '${_brands.length} Brand${_brands.length > 1 ? 's' : ''}' : 'Brand',
    ...
    return Row(children: [
      Expanded(child: TextField(controller: _searchCtrl, autofocus: true,
          ...
          decoration: InputDecoration(
              hintText: 'Search products...', hintStyle: TextStyle(color: _kT4,
              fontSize: (sw * 0.033).clamp(11.0, 14.0)),
              prefixIcon: Icon(Icons.search_rounded, color: _kT4, size: (sw * 0.048).clamp(16.0, 22.0)),
      ...
      SizedBox(width: sw * 0.02),
      CircularIconButton(icon: Icons.close_rounded,
  ...
  void _showBrandSheet(BuildContext context, double sw, double sh,
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
        ...
        builder: (_) => StatefulBuilder(builder: (ctx, setModal) => DraggableScrollableSheet(
            ...
            builder: (_, sc) => Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    Container(width: (sw * 0.09).clamp(32.0, 42.0),
                        ...
                        child: Icon(Icons.local_offer_outlined,
                            size: (sw * 0.045).clamp(15.0, 20.0), color: _kAmber)),
                    SizedBox(width: sw * 0.03),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Filter by Brand', style: TextStyle(
                          fontSize: (sw * 0.042).clamp(14.0, 19.0), fontWeight: FontWeight.w700, color: _kT1)),
                      Text('${temp.length} selected', style: TextStyle(
                          fontSize: (sw * 0.028).clamp(9.5, 12.5), color: _kT4)),
                    ])),
                    if (temp.isNotEmpty)
                      GestureDetector(onTap: () => setModal(() => temp = []),
                          child: Container(
                              padding: EdgeInsets.symmetric(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return GestureDetector(
                                  ...
                                  child: Container(
                                      ...
                                      decoration: BoxDecoration(color: sel ? _kAmberBg : _kBg,
                                          borderRadius: BorderRadius.circular((sw * 0.028).clamp(8.0, 12.0)),
                                          border: Border.all(color: sel ? _kAmberBd : _kBd, width: sel ? 1.0 : 0.5)),
                                      child: Row(children: [
                                        Container(
                                            width: (sw * 0.042).clamp(14.0, 20.0),
                                            height: (sw * 0.042).clamp(14.0, 20.0),
                                            decoration: BoxDecoration(color: sel ? _kAmber : _kWhite,
                                                shape: BoxShape.circle,
                                                border: Border.all(color: sel ? _kAmber : _kBd, width: 1.5)),
                                            child: sel ? Icon(Icons.check_rounded,
                                                size: (sw * 0.028).clamp(9.0, 12.0), color: _kWhite) : null),
                                        SizedBox(width: sw * 0.03),
                                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          ...
                                          if (b.brandModels.isNotEmpty)
                                            Text('${b.brandModels.length} model${b.brandModels.length > 1 ? 's' : ''}',
                                                style: TextStyle(fontSize: (sw * 0.027).clamp(9.0, 11.5), color: _kT4)),
                                        ])),
                                      ])));
                            });
                      })),
                  ...
                  SizedBox(width: double.infinity, child: ElevatedButton(
                      onPressed: () { setState(() => _brands = List<String>.from(temp)); Navigator.pop(context); },
                      ...
                      child: Text('Apply Filters', style: TextStyle(
                          fontSize: (sw * 0.038).clamp(13.0, 17.0), fontWeight: FontWeight.w700)))),
                ])))));
  }
  ...
  Widget _emptyView(double sw, double sh) {
    ...
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text(msg, style: TextStyle(fontSize: (sw * 0.036).clamp(12.0, 16.0),
          fontWeight: FontWeight.w600, color: _kT2), textAlign: TextAlign.center),
      ...
      if (_brands.isNotEmpty) ...[
        ...
        GestureDetector(onTap: () => setState(() => _brands = []),
            child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.close_rounded, size: (sw * 0.035).clamp(12.0, 16.0), color: _kAmber),
                  SizedBox(width: sw * 0.015),
                  Text('Clear brand filter', style: TextStyle(
      ...
      if (_brands.isEmpty && !_hasCategory && _tabIdx != 0) ...[
        ...
        GestureDetector(onTap: () => setState(() => _tabIdx = 0),
            child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.apps_rounded, size: (sw * 0.035).clamp(12.0, 16.0), color: _kP),
                  SizedBox(width: sw * 0.015),
                  Text('View all products', style: TextStyle(
                      fontSize: (sw * 0.03).clamp(10.0, 13.0), fontWeight: FontWeight.w700, color: _kP)),
                ]))),
      ],
    ]));
  }
    ...
    return Scaffold(backgroundColor: _kBg,
        body: SafeArea(child: Column(children: [
          ...
          Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Container(width: (sw * 0.18).clamp(60.0, 90.0), height: (sw * 0.18).clamp(60.0, 90.0),
                decoration: BoxDecoration(color: _kRedBg, shape: BoxShape.circle,
                    border: Border.all(color: _kRedBd, width: 0.5)),
                child: Icon(Icons.wifi_off_rounded, size: (sw * 0.09).clamp(30.0, 44.0), color: _kRed)),
            SizedBox(height: sh * 0.02),
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                        ...
                        decoration: BoxDecoration(color: _kPBg,
                            borderRadius: BorderRadius.circular((sw * 0.025).clamp(8.0, 12.0)),
                            border: Border.all(color: _kPBd, width: 0.5)),
                        child: Center(child: SvgPicture.asset(AppIcons.product,
                            width: (sw * 0.042).clamp(14.0, 20.0),
                    ...
                    Container(
                        ...
                        decoration: BoxDecoration(color: active ? _kGreenBg : _kRedBg,
                            borderRadius: BorderRadius.circular(20),
                            border: Border.all(color: active ? _kGreenBd : _kRedBd, width: 0.5)),
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          Container(width: (sw * 0.014).clamp(4.0, 7.0),
                              height: (sw * 0.014).clamp(4.0, 7.0),
                              decoration: BoxDecoration(color: active ? _kGreen : _kRed, shape: BoxShape.circle)),
                          SizedBox(width: sw * 0.008),
                  ...
                  Row(children: [
                    RoleGuard(feature: AppFeature.viewStock,
                        child: Row(children: [
                          _StockPill(sw: sw, label: 'P', value: product.packedStock?.toString() ?? '0',
                              color: _kGreen, bg: _kGreenBg),
                          SizedBox(width: sw * 0.012),
                          _StockPill(sw: sw, label: 'U', value: product.unpackedStock?.toString() ?? '0',
                              color: _kRed, bg: _kRedBg),
                          SizedBox(width: sw * 0.012),
                          _StockPill(sw: sw, label: 'T', value: product.availableStock?.toString() ?? '0',
                              color: _kP, bg: _kPBg),
                        ])),
                    const Spacer(),
                    RoleGuard(feature: AppFeature.viewPrice,
                        child: Text('₹${product.price ?? '0'}', style: TextStyle(
                            fontSize: (sw * 0.038).clamp(13.0, 17.0),
                            fontWeight: FontWeight.w800, color: _kP, letterSpacing: -0.3))),
                  ]),
                ]))));
  }
}
...
class _Tag extends StatelessWidget {
  const _Tag({required this.sw, required this.label, required this.color, required this.bg});
  final double sw; final String label; final Color color, bg;
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis,
          style: TextStyle(fontSize: (sw * 0.025).clamp(8.5, 11.0),
              fontWeight: FontWeight.w600, color: color)));
}

// ── Stock pill ────────────────────────────────────────────────────────────────
class _StockPill extends StatelessWidget {
  const _StockPill({required this.sw, required this.label,
    required this.value, required this.color, required this.bg});
  final double sw; final String label, value; final Color color, bg;
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Text(value, style: TextStyle(
            fontSize: (sw * 0.028).clamp(9.5, 12.5), fontWeight: FontWeight.w800, color: color)),
        SizedBox(width: sw * 0.006),
        Text(label, style: TextStyle(
            fontSize: (sw * 0.022).clamp(7.5, 10.0), fontWeight: FontWeight.w500,
