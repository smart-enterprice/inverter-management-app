...
class _DetailViewState extends ConsumerState<_DetailView> {
  ...
  Widget _stockCard(double sw, double sh, ProductModel p, bool active) {
    ...
    return _Section(sw: sw, icon: Icons.inventory_outlined, iconBg: _kAmberBg,
        ...
        child: Row(children: [
          Expanded(child: _StockTile(sw: sw, label: 'Packed', value: p.packedStock,
              icon: Icons.check_box_outlined, color: _kP)),
          if (!isBattery) ...[
            SizedBox(width: sw * 0.03),
            Expanded(child: _StockTile(sw: sw, label: 'Unpacked', value: p.unpackedStock,
...
class _PricingTilesState extends State<_PricingTiles> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      // ── Cost Price (Admin / Super Admin only) ────────────────────────
      RoleGuard(
        feature: AppFeature.viewCostPrice,
        child: Padding(
