...
class _OrderViewPageState extends ConsumerState<OrderViewPage> {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: sw * 0.02, runSpacing: sw * 0.015, children: [
          ...
          _Pill(sw: sw,
              label: 'Ordered: ${(item.qtyOrdered ?? 0)
                  + (item.qtyDelivered ?? 0)
                  + (item.totalCancelledQty ?? 0)}',
          ...
          Row(children: [
            ...
            Expanded(child: Text(
                ...
                style: TextStyle(
                    ...
                    color: _kT2, fontWeight: FontWeight.w700))),
            if (!isCancelled && !isCompleted && !isOrderLocked)
              OutlinedButton.icon(
                ...
                style: OutlinedButton.styleFrom(
                  foregroundColor: _kP,
                  side: const BorderSide(color: _kPBd, width: 0.8),
                  padding: EdgeInsets.symmetric(
                      horizontal: sw * 0.035, vertical: sw * 0.02),
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(
                          (sw * 0.025).clamp(7.0, 10.0))),
        ...
        // Delivery Qty button — visible whenever not cancelled / order not locked
        // and there's at least one non-cancelled unit. Lets user edit/correct
        // already-delivered qty too.
        if (!isCancelled && !isOrderLocked && maxDeliverable > 0) ...[
          SizedBox(height: sw * 0.02),
          if (isCompleted) Divider(height: 1, color: _kBd),
            ...
            Expanded(child: RoleGuard(
              ...
              child: _ActionBtn(sw: sw, label: 'Delivery Qty',
                  ...
                  onTap: () => _showDeliveryQtyDialog(
                      context, item, order, maxDeliverable)),
            )),
          ]),
        ],
      ]),
  ...
  void _showDeliveryQtyDialog(BuildContext context, OrderDetailsModel item,
      OrderModel order, int max) {
    if (item.deliveryDate == null) {
      _snack('Set delivery date first', _kAmber);
      return;
    }
    showDialog(
      context: context,
      builder: (_) => _DeliveryQtyDialog(
        item: item, maxDeliverable: max,
        onConfirm: (qty) async {
          await _submitDeliveredQty(item: item, order: order, qty: qty);
        },
      ),
    );
  }
  ...
  Future<void> _submitDeliveredQty({
    required OrderDetailsModel item, required OrderModel order,
    required int qty,
  }) async {
    final updatedItem = item.copyWith(
      deliveredQty: qty,
      isDeliveredQtyUpdated: true,
      isDeliveryDateUpdated: false,
    ...
    await ref.read(orderControllerProvider.notifier).updateOrderItemStatus(
        current.copyWith(orderDetails: details));
    ref.invalidate(orderByIdProvider(widget.orderNumber));
    if (mounted) _snack('$qty unit(s) delivered', _kGreen);
  }
              ...
              Row(children: [
                Expanded(child: _QtyCard(sw: sw, label: 'Ordered',
                    value: '${(widget.item.qtyOrdered ?? 0)
                        + (widget.item.qtyDelivered ?? 0)
                        + (widget.item.totalCancelledQty ?? 0)}',
...
class _DeliveryQtyDialog extends StatefulWidget {
  final OrderDetailsModel item;
  final int maxDeliverable;
  final Future<void> Function(int qty) onConfirm;
  const _DeliveryQtyDialog({required this.item, required this.maxDeliverable,
    required this.onConfirm});
  @override State<_DeliveryQtyDialog> createState() => _DeliveryQtyDialogState();
}

class _DeliveryQtyDialogState extends State<_DeliveryQtyDialog> {
  final _qtyCtrl = TextEditingController();
  int? _qty;
  late final int _initialDelivered;
  bool _loading = false;

  @override
  void initState() {
    super.initState();
    _initialDelivered = widget.item.qtyDelivered ?? 0;
    if (_initialDelivered > 0) {
      _qty = _initialDelivered;
      _qtyCtrl.text = _initialDelivered.toString();
    }
  }

  @override void dispose() { _qtyCtrl.dispose(); super.dispose(); }

  // Allow 0..max so user can correct mistakes downward too.
  // Submit button is also disabled if qty equals the existing value (no change).
  bool get _validQty =>
      _qty != null && _qty! >= 0 && _qty! <= widget.maxDeliverable;
  bool get _changed => _qty != null && _qty != _initialDelivered;

  @override
  Widget build(BuildContext context) {
    final sw = MediaQuery.sizeOf(context).width;
    final sh = MediaQuery.sizeOf(context).height;
    final dateLabel = widget.item.deliveryDate != null
        ? DateFormat('dd MMM yyyy').format(widget.item.deliveryDate!)
        : '—';
    return Dialog(
      backgroundColor: _kWhite,
      shape: RoundedRectangleBorder(
      ...
      child: SingleChildScrollView(child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min,
            ...
            children: [
              Row(children: [
                _IconBox(sw: sw, icon: Icons.local_shipping_outlined,
                    bg: _kGreenBg, color: _kGreen),
                SizedBox(width: sw * 0.025),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Delivery Quantity', style: TextStyle(
                      fontSize: (sw * 0.042).clamp(14.0, 19.0),
                      fontWeight: FontWeight.w800, color: _kT1)),
                  Text(widget.item.productName, style: TextStyle(
                      fontSize: (sw * 0.03).clamp(10.0, 13.0), color: _kT4),
              ...
              Row(children: [
                ...
                Expanded(child: _QtyCard(sw: sw, label: 'Delivered',
                    ...
                    color: _kGreen, bg: _kGreenBg, bd: _kGreenBd)),
                SizedBox(width: sw * 0.02),
                Expanded(child: _QtyCard(sw: sw, label: 'Max',
                    value: '${widget.maxDeliverable}',
                    color: _kAmber, bg: _kAmberBg, bd: _kAmberBd)),
              ]),
              SizedBox(height: sh * 0.02),
              Container(
              ...
              TextField(
                ...
                style: TextStyle(fontSize: (sw * 0.036).clamp(12.0, 16.0), color: _kT1),
                decoration: InputDecoration(
                    hintText: 'Qty (Max: ${widget.maxDeliverable})',
                    hintStyle: TextStyle(color: _kT4),
                    filled: true, fillColor: _kBg,
                    contentPadding: EdgeInsets.symmetric(
                        horizontal: sw * 0.04, vertical: sw * 0.035),
                    ...
                    focusedBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.circular((sw * 0.028).clamp(8.0, 12.0)),
                        borderSide: const BorderSide(color: _kP, width: 1.5)),
                    errorText: _qty != null && _qty! > widget.maxDeliverable
                        ? 'Cannot exceed ${widget.maxDeliverable}'
                        : _qty != null && _qty! < 0 ? 'Must be ≥ 0' : null),
              ),
              SizedBox(height: sh * 0.025),
              Row(children: [
                ...
                SizedBox(width: sw * 0.03),
                Expanded(child: ElevatedButton(
                    onPressed: !_validQty || !_changed || _loading ? null : () async {
                      setState(() => _loading = true);
                      try {
...
class _DeliveryDateDialogState extends State<_DeliveryDateDialog> {
  ...
  void initState() {
    super.initState();
    _date = widget.item.deliveryDate ?? DateTime.now();
  }
