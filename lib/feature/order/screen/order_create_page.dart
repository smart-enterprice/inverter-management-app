...
class OrderCreatePage extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<OrderCreatePage> createState() => _OrderCreatePageState();
}
...
class _OrderCreatePageState extends ConsumerState<OrderCreatePage> {
  ...
  @override void initState() { super.initState(); _loadCurrentUserInfo(); }
  ...
  Future<void> _loadCurrentUserInfo() async {
    try {
      ...
      setState(() { currentUserRole = role; currentUserId = userId; isLoadingUserInfo = false; });
      if (role == 'ROLE_SALESMAN' && userId != null) _loadSalesmanInfo(userId);
    } catch (e) { debugPrint('Error loading user info: $e'); setState(() => isLoadingUserInfo = false); }
  }
  ...
  Future<void> _loadSalesmanInfo(String salesmanId) async {
    ...
    } catch (e) { debugPrint('Error loading salesman info: $e'); }
  }

  bool get isSalesman => currentUserRole == 'ROLE_SALESMAN';

  @override void dispose() { orderNoteController.dispose(); amountPaidController.dispose(); super.dispose(); }
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: SingleChildScrollView(
            padding: EdgeInsets.fromLTRB(sw * 0.038, sh * 0.012, sw * 0.038, sh * 0.02),
  ...
  Widget _buildSelectionCard(double sw, double sh) {
    return _Section(sw: sw, icon: Icons.shopping_cart_outlined, iconBg: _kPBg,
        iconColor: _kP, title: 'Order Selection',
        child: Column(children: [
          if (!isSalesman) ...[
            _SelectorTile(sw: sw, label: 'Salesman',
                value: selectedSalesman?.employeeName,
                icon: Icons.person_outline_rounded, color: _kPurple,
                onTap: () => _showSalesmanDialog(context)),
            SizedBox(height: sh * 0.01),
          ],
          _SelectorTile(sw: sw, label: 'Dealer',
              value: selectedDealer != null
                  ? '${selectedDealer!.employeeName}${selectedDealer!.shopName != null ? ' (${selectedDealer!.shopName})' : ''}'
                  : null,
              icon: Icons.store_outlined, color: _kP,
              onTap: () => _showDealerDialog(context)),
          SizedBox(height: sh * 0.01),
          _SelectorTile(sw: sw, label: 'Brand',
              value: selectedBrand?.brandName,
              icon: Icons.local_offer_outlined, color: _kAmber,
              enabled: selectedDealer != null,
              onTap: selectedDealer == null ? null
                  : () => _showBrandDialog(context, selectedDealer!.employeeId!)),
          SizedBox(height: sh * 0.01),
          _SelectorTile(sw: sw, label: 'Model',
              value: selectedModelFilter, fallback: 'All Models',
              icon: Icons.tag_rounded, color: _kP,
              enabled: selectedBrand != null,
              onTap: selectedBrand == null ? null : () => _showModelDialog(context)),
          SizedBox(height: sh * 0.01),
          _SelectorTile(sw: sw, label: 'Product',
              value: selectedProduct?.productName,
              icon: Icons.inventory_2_outlined, color: _kGreen,
              enabled: selectedBrand != null,
              onTap: selectedBrand == null ? null
  ...
  Widget _buildProductsCard(double sw, double sh) {
    return _Section(sw: sw, icon: Icons.inventory_outlined, iconBg: _kGreenBg,
        iconColor: _kGreen, title: 'Selected Products',
        trailing: _CountBadge(count: selectedProducts.length, color: _kGreen, bg: _kGreenBg, bd: _kGreenBd),
        bottomChild: Container(
            padding: EdgeInsets.all(sw * 0.04),
            decoration: BoxDecoration(color: _kGreenBg,
                borderRadius: BorderRadius.only(
                    bottomLeft: Radius.circular((sw * 0.04).clamp(10.0, 18.0)),
                    bottomRight: Radius.circular((sw * 0.04).clamp(10.0, 18.0))),
                border: const Border(top: BorderSide(color: _kGreenBd, width: 0.5))),
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Order Total', style: TextStyle(
                  fontSize: (sw * 0.036).clamp(12.0, 16.0), fontWeight: FontWeight.w800, color: _kGreen)),
              Text('₹${_calculateOrderTotal().toStringAsFixed(2)}', style: TextStyle(
                  fontSize: (sw * 0.042).clamp(14.0, 20.0), fontWeight: FontWeight.w900, color: _kGreen)),
            ])),
        child: Column(children: [
          ListView.separated(
              ...
              itemBuilder: (_, i) => _buildProductItem(selectedProducts[i], i, sw, sh)),
        ]));
  }

  Widget _buildProductItem(OrderDetailsModel product, int index, double sw, double sh) {
    double price = double.tryParse(product.product?.price?.toString() ?? '0') ?? 0;
    ...
    if (product.useDealerDiscount && discount != null) {
      unitDiscount = discount.isPercentage == true
          ? price * (discount.discountValue / 100) : discount.discountValue.toDouble();
    ...
    return Padding(padding: EdgeInsets.all(sw * 0.04),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                _Chip(sw: sw, text: product.product?.brand ?? product.productBrand,
                    icon: Icons.local_offer_outlined, color: _kAmber),
                SizedBox(width: sw * 0.015),
                _Chip(sw: sw, text: product.product?.model ?? product.productModel,
                    icon: Icons.tag_rounded, color: _kP),
              ]),
            ])),
          ...
          Row(children: [
            Text('Quantity', style: TextStyle(
                fontSize: (sw * 0.03).clamp(10.0, 13.0), fontWeight: FontWeight.w600, color: _kT4)),
            SizedBox(width: sw * 0.025),
            _QtyBtn(sw: sw, icon: Icons.remove,
                onTap: product.quantity > 1 ? () => setState(() {
                  selectedProducts[index] = product.copyWith(qtyOrdered: product.quantity - 1);
                }) : null),
            ...
            Container(
                ...
                child: Text('${product.quantity}', style: TextStyle(
                    fontSize: (sw * 0.036).clamp(12.0, 16.0), fontWeight: FontWeight.w700, color: _kP))),
            SizedBox(width: sw * 0.02),
            _QtyBtn(sw: sw, icon: Icons.add, onTap: () => setState(() {
              selectedProducts[index] = product.copyWith(qtyOrdered: product.quantity + 1);
            })),
            const Spacer(),
          ...
          // Scheme toggle
          Row(children: [
            Icon(Icons.card_giftcard_outlined, size: (sw * 0.045).clamp(15.0, 20.0), color: _kPurple),
            SizedBox(width: sw * 0.02),
            Text('Scheme Product', style: TextStyle(
                fontSize: (sw * 0.032).clamp(11.0, 14.0), fontWeight: FontWeight.w600, color: _kT1)),
            const Spacer(),
            Switch(value: product.isScheme, activeColor: _kPurple, activeTrackColor: _kPurpleBg,
                onChanged: (v) => setState(() { selectedProducts[index] = product.copyWith(isProductScheme: v); })),
          ]),

          // Discount
          if (selectedDealer != null) _buildDiscountOptions(product, index, sw, sh),
          SizedBox(height: sh * 0.015),
          ...
          GestureDetector(
              ...
              child: Container(
                  ...
                  decoration: BoxDecoration(color: _kBg,
                      ...
                      border: Border.all(color: product.deliveryDate == null ? _kRed : _kBd, width: 0.5)),
                  child: Row(children: [
                    Icon(Icons.calendar_today_outlined,
                        size: (sw * 0.04).clamp(13.0, 18.0),
                        color: product.deliveryDate != null ? _kP : _kT4),
                    SizedBox(width: sw * 0.025),
                    Text(product.deliveryDate != null
                        ? '${product.deliveryDate!.day}/${product.deliveryDate!.month}/${product.deliveryDate!.year}'
                        : 'Select Delivery Date',
                        style: TextStyle(fontSize: (sw * 0.032).clamp(11.0, 14.0),
                            fontWeight: FontWeight.w500,
                            color: product.deliveryDate != null ? _kT1 : _kT4)),
                  ]))),
          if (product.deliveryDate == null)
            Padding(padding: EdgeInsets.only(top: sw * 0.015, left: sw * 0.01),
                child: Row(children: [
                  Icon(Icons.warning_amber_rounded, size: (sw * 0.035).clamp(12.0, 16.0), color: _kRed),
                  SizedBox(width: sw * 0.01),
                  Text('Delivery date required', style: TextStyle(
                      fontSize: (sw * 0.028).clamp(9.5, 12.5), color: _kRed)),
                ])),
          ...
          if (!product.isProductScheme) ...[
            ...
            Container(
                ...
                child: Column(children: [
                  ...
                  SizedBox(height: sw * 0.01),
                  _PriceRow(sw: sw, label: 'Total', amount: total, isBold: true),
                ])),
          ],
        ]));
  }
  ...
  Widget _buildOrderDetailsCard(double sw, double sh) {
    return _Section(sw: sw, icon: Icons.description_outlined, iconBg: _kPurpleBg,
        iconColor: _kPurple, title: 'Order Details',
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          // Notes
          Text('Order Notes', style: TextStyle(
              fontSize: (sw * 0.032).clamp(11.0, 14.0), fontWeight: FontWeight.w600, color: _kT1)),
          SizedBox(height: sh * 0.008),
          TextFormField(controller: orderNoteController, minLines: 3, maxLines: 5,
              style: TextStyle(fontSize: (sw * 0.034).clamp(11.5, 15.0), color: _kT1),
              decoration: _inputDeco(sw, 'Add special instructions or notes (optional)')),
          SizedBox(height: sh * 0.02),
          ...
          Row(children: priorities.map((p) {
            ...
            final c = p == 'HIGH' ? _kRed : p == 'MEDIUM' ? _kAmber : _kGreen;
            return Expanded(child: GestureDetector(
                ...
                child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(color: sel ? c : _kBd, width: sel ? 1.0 : 0.5)),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(sel ? Icons.radio_button_checked : Icons.radio_button_unchecked,
                          color: c, size: (sw * 0.045).clamp(15.0, 20.0)),
                      SizedBox(width: sw * 0.015),
                      Text(p, style: TextStyle(fontSize: (sw * 0.03).clamp(10.0, 13.0),
          ...
          Text('Amount Paid', style: TextStyle(
              fontSize: (sw * 0.032).clamp(11.0, 14.0), fontWeight: FontWeight.w600, color: _kT1)),
          SizedBox(height: sh * 0.008),
          TextFormField(controller: amountPaidController, keyboardType: TextInputType.number,
              style: TextStyle(fontSize: (sw * 0.036).clamp(12.0, 16.0), color: _kT1),
              decoration: _inputDeco(sw, 'Enter amount',
                  prefix: Icon(Icons.currency_rupee, color: _kGreen, size: (sw * 0.045).clamp(15.0, 20.0))),
              onChanged: (v) => setState(() => amountPaid = num.tryParse(v) ?? 0)),
          SizedBox(height: sh * 0.02),
          ...
          SizedBox(height: sh * 0.012),
          Row(children: [
            _PayMethodBtn(sw: sw, label: 'Cash', icon: Icons.payments_outlined,
                color: _kGreen, bg: _kGreenBg, bd: _kGreenBd,
                selected: paymentMethod == 'CASH',
                onTap: () => setState(() => paymentMethod = 'CASH')),
            SizedBox(width: sw * 0.02),
            _PayMethodBtn(sw: sw, label: 'Bank', icon: Icons.account_balance_outlined,
        ...
        child: SafeArea(top: false,
            child: SizedBox(width: double.infinity,
                child: ElevatedButton(
                    onPressed: canCreate && !isCreatingOrder ? _createOrder : null,
                    ...
                    child: isCreatingOrder
                        ? SizedBox(height: (sw * 0.05).clamp(16.0, 22.0),
  ...
  // Discount Options
  // ══════════════════════════════════════════════════════════════════════════
  Widget _buildDiscountOptions(OrderDetailsModel product, int index, double sw, double sh) {
    final discount = product.dealerDiscount;

    if (discount == null) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        SizedBox(height: sh * 0.015),
        Text('Manual Discount', style: TextStyle(
        ...
        TextFormField(
            ...
            decoration: _inputDeco(sw, 'Enter discount amount',
                prefix: Icon(Icons.local_offer_outlined, color: _kAmber, size: (sw * 0.045).clamp(15.0, 20.0))),
            onChanged: (v) => setState(() {
              selectedProducts[index] = product.copyWith(
                  discountAmount: num.tryParse(v), useDealerDiscount: false);
            })),
      ]);
    }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: sh * 0.01),
      Row(children: [
        Expanded(child: _ToggleBtn(sw: sw, label: 'Manual', color: _kAmber,
            bg: _kAmberBg, bd: _kAmberBd, selected: !product.useDealerDiscount,
            onTap: () => setState(() {
              selectedProducts[index] = product.copyWith(
                  useDealerDiscount: false, dealerDiscountId: null, discountAmount: 0);
            }))),
        SizedBox(width: sw * 0.02),
        Expanded(child: _ToggleBtn(sw: sw, label: 'Dealer', color: _kGreen,
            bg: _kGreenBg, bd: _kGreenBd, selected: product.useDealerDiscount,
            onTap: () => setState(() {
              selectedProducts[index] = product.copyWith(
                  useDealerDiscount: true, discountAmount: null,
      ...
      if (!product.useDealerDiscount) ...[
        ...
        TextFormField(
            ...
            decoration: _inputDeco(sw, 'Enter amount'),
            onChanged: (v) => setState(() {
              selectedProducts[index] = product.copyWith(discountAmount: num.tryParse(v) ?? 0);
            })),
      ],
      if (product.useDealerDiscount) ...[
        SizedBox(height: sh * 0.01),
        Container(padding: EdgeInsets.all(sw * 0.035),
            decoration: BoxDecoration(color: _kGreenBg,
                borderRadius: BorderRadius.circular((sw * 0.028).clamp(8.0, 12.0)),
                border: Border.all(color: _kGreenBd, width: 0.5)),
            child: Row(children: [
              Icon(Icons.discount_outlined, color: _kGreen, size: (sw * 0.045).clamp(15.0, 20.0)),
              SizedBox(width: sw * 0.025),
              Text('${discount.isPercentage == false ? '₹' : ''}${discount.discountValue}${discount.isPercentage ? '%' : ''}',
                  style: TextStyle(fontSize: (sw * 0.036).clamp(12.0, 16.0),
  ...
  InputDecoration _inputDeco(double sw, String hint, {Widget? prefix}) {
    ...
    return InputDecoration(
        hintText: hint,
        hintStyle: TextStyle(fontSize: (sw * 0.032).clamp(11.0, 14.0), color: _kT4),
        prefixIcon: prefix,
  ...
  Future<void> _showDealerDialog(BuildContext context) async {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
        ...
        title: Text('Select Dealer', style: TextStyle(
            fontSize: (sw * 0.042).clamp(14.0, 20.0), fontWeight: FontWeight.w700, color: _kT1)),
        content: SizedBox(width: double.maxFinite, child: Consumer(builder: (_, ref, __) {
          final dealerAsync = ref.watch(dealerListProvider);
          return dealerAsync.when(
              data: (dealers) {
                final query = ValueNotifier('');
                return Column(mainAxisSize: MainAxisSize.min, children: [
                  _dialogSearchField(ctx, searchController, 'Search by name or shop', query),
                  SizedBox(height: sh * 0.02),
                      ...
                      builder: (_, q, __) {
                        ...
                        return ListView.builder(itemCount: filtered.length, itemBuilder: (_, i) {
                          ...
                          return _dialogListItem(ctx, title: d.employeeName,
                              subtitle: d.shopName ?? 'No shop name',
                              onTap: () {
                                setState(() { selectedDealer = d; selectedBrand = null;
                                selectedModelFilter = null; selectedProduct = null;
                                selectedProducts.clear(); _allFetchedProducts.clear();
  ...
  Future<void> _showSalesmanDialog(BuildContext context) async {
    ...
    showDialog(context: context, builder: (ctx) => FutureBuilder<List<UserModel>>(
        future: future,
        builder: (_, snap) {
          ...
          if (snap.hasError) return AlertDialog(title: const Text('Salesmen'), content: Text(snap.error.toString()));
          final salesmen = snap.data ?? [];
          final query = ValueNotifier('');
          return AlertDialog(backgroundColor: _kWhite,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular((sw * 0.04).clamp(10.0, 18.0))),
              title: Text('Select Salesman', style: TextStyle(
                  fontSize: (sw * 0.042).clamp(14.0, 20.0), fontWeight: FontWeight.w700, color: _kT1)),
              content: SizedBox(width: double.maxFinite, child: Column(mainAxisSize: MainAxisSize.min, children: [
                _dialogSearchField(ctx, searchController, 'Search by name or phone', query),
                SizedBox(height: sh * 0.02),
                Expanded(child: ValueListenableBuilder<String>(valueListenable: query, builder: (_, q, __) {
                  final filtered = salesmen.where((s) {
                    return s.employeeName.toLowerCase().contains(q) || s.employeePhone.toLowerCase().contains(q);
                  }).toList();
                  if (filtered.isEmpty) return Center(child: Text('No salesmen found',
                      style: TextStyle(color: _kT4, fontSize: (sw * 0.034).clamp(11.5, 15.0))));
                  return ListView.builder(itemCount: filtered.length, itemBuilder: (_, i) {
                    final s = filtered[i];
                    return _dialogListItem(ctx, title: s.employeeName, subtitle: s.employeePhone,
                        onTap: () { setState(() => selectedSalesman = s); Navigator.pop(ctx); });
                  });
                })),
              ])));
        }));
  ...
  Future<void> _showModelDialog(BuildContext context) async {
    ...
    if (_allFetchedProducts.isEmpty) {
      _allFetchedProducts = await ref.read(productControllerProvider.notifier)
          .fetchProductsByBrand([selectedBrand!.brandName]);
    }
    _availableModels = [...<String>{}..addAll(
        _allFetchedProducts.map((p) => p.model ?? '').where((m) => m.isNotEmpty))]..sort();
    final query = ValueNotifier('');
    if (!context.mounted) return;
    showDialog(context: context, builder: (ctx) => AlertDialog(
        backgroundColor: _kWhite,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular((sw * 0.04).clamp(10.0, 18.0))),
        title: Text('Select Model', style: TextStyle(
            fontSize: (sw * 0.042).clamp(14.0, 20.0), fontWeight: FontWeight.w700, color: _kT1)),
        content: SizedBox(width: double.maxFinite, child: Column(mainAxisSize: MainAxisSize.min, children: [
          _dialogSearchField(ctx, searchController, 'Search model', query),
          SizedBox(height: sh * 0.015),
          Expanded(child: ValueListenableBuilder<String>(valueListenable: query, builder: (_, q, __) {
            final filtered = _availableModels.where((m) => m.toLowerCase().contains(q)).toList();
            return ListView.builder(itemCount: filtered.length + 1, itemBuilder: (_, i) {
              if (i == 0) {
                final sel = selectedModelFilter == null;
                return _dialogSelectItem(ctx, sw, title: 'All Models',
                    icon: Icons.all_inclusive_rounded, selected: sel,
                    onTap: () { setState(() { selectedModelFilter = null; selectedProduct = null; });
                    Navigator.pop(ctx); });
              }
              final model = filtered[i - 1];
              final sel = selectedModelFilter == model;
              return _dialogSelectItem(ctx, sw, title: model,
                  icon: Icons.tag_rounded, selected: sel,
                  onTap: () { setState(() { selectedModelFilter = model; selectedProduct = null; });
                  Navigator.pop(ctx); });
            });
          })),
        ]))));
  ...
  Future<void> _showProductDialog(BuildContext context, List<String> brands) async {
    final searchController = TextEditingController();
    final sw = MediaQuery.sizeOf(context).width;
    final sh = MediaQuery.sizeOf(context).height;
    if (_allFetchedProducts.isEmpty) {
      _allFetchedProducts = await ref.read(productControllerProvider.notifier).fetchProductsByBrand(brands);
    }
    final localFiltered = _allFetchedProducts.where((p) {
      final matchBrand = brands.any((b) => b.toLowerCase() == (p.brand ?? '').toLowerCase());
      final matchModel = selectedModelFilter == null || (p.model ?? '') == selectedModelFilter;
      return matchBrand && matchModel;
    }).toList();
    final models = ['All', ...{...localFiltered.map((p) => p.model ?? '')}.where((m) => m.isNotEmpty).toList()..sort()];
    final query = ValueNotifier('');
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
        backgroundColor: _kWhite,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular((sw * 0.04).clamp(10.0, 18.0))),
        title: Text('Select Product', style: TextStyle(
            fontSize: (sw * 0.042).clamp(14.0, 20.0), fontWeight: FontWeight.w700, color: _kT1)),
        content: SizedBox(width: double.maxFinite, child: Column(mainAxisSize: MainAxisSize.min, children: [
          _dialogSearchField(ctx, searchController, 'Search product name or model', query),
          SizedBox(height: sh * 0.015),
          // Model chips
          SizedBox(height: sw * 0.09, child: ValueListenableBuilder<String>(
              valueListenable: selectedModel, builder: (_, cur, __) {
            return ListView.builder(scrollDirection: Axis.horizontal, itemCount: models.length,
                itemBuilder: (_, i) {
                  final m = models[i]; final sel = cur == m;
                  return GestureDetector(onTap: () => selectedModel.value = m,
                      child: Container(
          ...
          // Product list
          Expanded(child: ValueListenableBuilder<String>(valueListenable: query, builder: (_, q, __) {
            return ValueListenableBuilder<String>(valueListenable: selectedModel, builder: (_, cur, __) {
              final filtered = localFiltered.where((p) {
                final name = p.productName?.toLowerCase() ?? '';
                final model = p.model?.toLowerCase() ?? '';
                final matchSearch = name.contains(q) || model.contains(q);
                final matchModel = cur == 'All' || (p.model ?? '') == cur;
              ...
              return ListView.builder(itemCount: filtered.length, itemBuilder: (_, i) {
                ...
                return Container(
                    ...
                    decoration: BoxDecoration(color: added ? _kGreenBg : _kBg,
                        borderRadius: BorderRadius.circular((sw * 0.028).clamp(8.0, 12.0)),
                        border: Border.all(color: added ? _kGreenBd : _kBd, width: 0.5)),
                    child: ListTile(
                        leading: SvgPicture.asset(AppIcons.product, width: sw * 0.06,
                            colorFilter: ColorFilter.mode(added ? _kGreen : _kP, BlendMode.srcIn)),
                        title: Text(p.productName.toString(),
                            style: TextStyle(fontWeight: FontWeight.w600, color: _kT1,
                                fontSize: (sw * 0.034).clamp(11.5, 15.0))),
                        subtitle: Text(p.model.toString(),
                            style: TextStyle(color: _kT4, fontSize: (sw * 0.028).clamp(9.5, 12.5))),
                        trailing: added ? const Icon(Icons.check_circle, color: _kGreen) : null,
                        onTap: () async {
                          DealerDiscountModel? discount;
                          try { discount = await ref.read(dealerProductDiscountProvider({
                            'dealerId': selectedDealer!.employeeId!, 'productId': p.productId!,
                          }).future); } catch (e) { debugPrint('Discount fetch: $e'); }
                          setState(() { selectedProducts.add(
                              OrderDetailsModel.fromProduct(p, dealerDiscount: discount)); });
  ...
  Widget _dialogSearchField(BuildContext ctx, TextEditingController ctrl,
    ...
    return TextField(controller: ctrl,
        ...
        decoration: InputDecoration(hintText: hint,
            hintStyle: TextStyle(fontSize: (sw * 0.032).clamp(11.0, 14.0), color: _kT4),
            prefixIcon: Icon(Icons.search_rounded, size: (sw * 0.05).clamp(16.0, 22.0), color: _kT4),
            filled: true, fillColor: _kBg,
  ...
  Widget _dialogListItem(BuildContext ctx, {required String title,
    String? subtitle, Widget? leading, required VoidCallback onTap}) {
    final sw = MediaQuery.sizeOf(ctx).width;
    return Container(
        margin: EdgeInsets.only(bottom: sw * 0.02),
        decoration: BoxDecoration(color: _kBg,
        ...
        child: ListTile(leading: leading,
            title: Text(title, style: TextStyle(fontWeight: FontWeight.w600, color: _kT1,
                fontSize: (sw * 0.034).clamp(11.5, 15.0))),
            subtitle: subtitle != null ? Text(subtitle,
                style: TextStyle(color: _kT4, fontSize: (sw * 0.028).clamp(9.5, 12.5))) : null,
            onTap: onTap));
  }

  Widget _dialogSelectItem(BuildContext ctx, double sw, {required String title,
    required IconData icon, required bool selected, required VoidCallback onTap}) {
    return Container(
        margin: EdgeInsets.only(bottom: sw * 0.02),
        decoration: BoxDecoration(color: selected ? _kPBg : _kBg,
            borderRadius: BorderRadius.circular((sw * 0.028).clamp(8.0, 12.0)),
            border: Border.all(color: selected ? _kPBd : _kBd, width: selected ? 1.0 : 0.5)),
        child: ListTile(
            leading: Icon(icon, color: selected ? _kP : _kT4),
            title: Text(title, style: TextStyle(fontWeight: FontWeight.w600,
                color: selected ? _kP : _kT1, fontSize: (sw * 0.034).clamp(11.5, 15.0))),
            trailing: selected ? const Icon(Icons.check_circle, color: _kP) : null,
            onTap: onTap));
  }
  ...
  Future<void> _selectDeliveryDate(BuildContext context,
    ...
    final picked = await showDatePicker(context: context,
        ...
        builder: (ctx, child) => Theme(data: Theme.of(ctx).copyWith(
            colorScheme: const ColorScheme.light(primary: _kP, onPrimary: _kWhite,
                surface: _kWhite, onSurface: _kT1),
            dialogTheme: const DialogThemeData(backgroundColor: _kWhite)), child: child!));
    if (picked != null) setState(() { selectedProducts[index] = product.copyWith(deliveryDate: picked); });
  }
  ...
  bool _canCreateOrder() {
    if (selectedDealer == null || selectedSalesman == null ||
        selectedProducts.isEmpty || amountPaid < 0) return false;
  ...
  void _createOrder() async {
    ...
    if (missingDate) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Row(children: [
            ...
            SizedBox(width: 12),
            Expanded(child: Text('Please select delivery date for all products')),
          ]),
    ...
    try {
      final processedProducts = selectedProducts.map((p) {
        ...
        if (p.useDealerDiscount && p.dealerDiscount != null) {
          ...
          calculatedDiscount = d.isPercentage == true
              ? unitPrice * (d.discountValue / 100) : d.discountValue.toDouble();
      ...
      final order = OrderModel(dealerId: selectedDealer!.employeeId!, priority: selectedPriority,
          orderNote: orderNoteController.text, salesmanId: selectedSalesman!.employeeId!,
          paymentType: paymentMethod, amountPaid: amountPaid, orderDetails: processedProducts);
      await ref.read(orderControllerProvider.notifier).createOrder(order);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Row(children: [
            Icon(Icons.check_circle, color: _kWhite, size: 20), SizedBox(width: 12),
            Expanded(child: Text('Order created successfully!', style: TextStyle(fontWeight: FontWeight.w500))),
          ]),
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Row(children: [
            const Icon(Icons.error_outline, color: _kWhite), const SizedBox(width: 12),
            Expanded(child: Text('Failed to create order: $e')),
          ]),
          backgroundColor: _kRed, behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))));
    } finally { if (mounted) setState(() => isCreatingOrder = false); }
  }
  ...
  double _calculateOrderTotal() {
    ...
    for (final p in selectedProducts) {
      ...
      if (p.useDealerDiscount && discount != null) {
        unitDiscount = discount.isPercentage == true
            ? unitPrice * (discount.discountValue / 100) : discount.discountValue.toDouble();
      } else if (!p.useDealerDiscount && p.discountAmount != null) {
...
class _Section extends StatelessWidget {
  const _Section({required this.sw, required this.icon, required this.iconBg,
    required this.iconColor, required this.title, required this.child,
    this.trailing, this.bottomChild});
    ...
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: EdgeInsets.symmetric(horizontal: sw * 0.04, vertical: sw * 0.035),
              child: Row(children: [
                Container(width: (sw * 0.075).clamp(26.0, 36.0),
...
class _SelectorTile extends StatelessWidget {
  const _SelectorTile({required this.sw, required this.label, this.value,
    required this.icon, required this.color, this.onTap, this.enabled = true,
    this.fallback});
  final double sw; final String label; final String? value;
  final IconData icon; final Color color; final VoidCallback? onTap;
  final bool enabled; final String? fallback;

  @override
  Widget build(BuildContext context) {
    final display = value ?? fallback ?? 'Select $label';
    final hasVal = value != null;
    return Opacity(opacity: enabled ? 1.0 : 0.5,
            ...
            child: Container(
                ...
                child: Row(children: [
                  Container(width: (sw * 0.09).clamp(32.0, 42.0), height: (sw * 0.09).clamp(32.0, 42.0),
                      ...
                      child: Icon(icon, color: color, size: (sw * 0.04).clamp(14.0, 20.0))),
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(display, style: TextStyle(
                        ...
                        color: hasVal ? _kT1 : _kT4), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ])),
...
class _CountBadge extends StatelessWidget {
  ...
  final int count; final Color color, bg, bd;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        ...
        child: Text('$count', style: TextStyle(
            fontSize: (sw * 0.026).clamp(9.0, 11.5), fontWeight: FontWeight.w700, color: color)));
  }
}

class _Chip extends StatelessWidget {
  const _Chip({required this.sw, required this.text, required this.icon, required this.color});
  final double sw; final String text; final IconData icon; final Color color;
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(width: sw * 0.01),
        Text(text, style: TextStyle(fontSize: (sw * 0.026).clamp(9.0, 11.5),
...
class _QtyBtn extends StatelessWidget {
  ...
  final double sw; final IconData icon; final VoidCallback? onTap;
...
class _PriceRow extends StatelessWidget {
  const _PriceRow({required this.sw, required this.label,
    required this.amount, this.isBold = false});
  final double sw; final String label; final double amount; final bool isBold;
...
class _ToggleBtn extends StatelessWidget {
  const _ToggleBtn({required this.sw, required this.label, required this.color,
    required this.bg, required this.bd, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) => GestureDetector(onTap: onTap,
      child: Container(
          padding: EdgeInsets.symmetric(vertical: sw * 0.025),
          decoration: BoxDecoration(color: selected ? bg : _kBg,
              borderRadius: BorderRadius.circular((sw * 0.025).clamp(8.0, 12.0)),
              border: Border.all(color: selected ? bd : _kBd, width: selected ? 1.0 : 0.5)),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(selected ? Icons.radio_button_checked : Icons.radio_button_unchecked,
                color: color, size: (sw * 0.045).clamp(15.0, 20.0)),
            SizedBox(width: sw * 0.015),
...
class _PayMethodBtn extends StatelessWidget {
  const _PayMethodBtn({required this.sw, required this.label, required this.icon,
    required this.color, required this.bg, required this.bd,
    required this.selected, required this.onTap});
  final double sw; final String label; final IconData icon;
  final Color color, bg, bd; final bool selected; final VoidCallback onTap;
  ...
  Widget build(BuildContext context) => Expanded(child: GestureDetector(onTap: onTap,
      child: Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(selected ? Icons.radio_button_checked : Icons.radio_button_unchecked,
                color: color, size: (sw * 0.05).clamp(16.0, 22.0)),
            SizedBox(width: sw * 0.02),
            Text(label, style: TextStyle(fontSize: (sw * 0.034).clamp(11.5, 15.0),
                fontWeight: FontWeight.w700, color: color)),
          ]))));
}